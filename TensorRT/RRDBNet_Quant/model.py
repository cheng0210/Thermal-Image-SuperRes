...
class ResidualDenseBlock(nn.Module):
    def __init__(self, nf, gc=32, res_scale=0.2, quantize:bool=True):
        super(ResidualDenseBlock, self).__init__()
        self.layer1 = nn.Sequential(QuantConv(nf + 0 * gc, gc, 3, padding=1, bias=True), nn.LeakyReLU(negative_slope=0.2, inplace=True))
        self.layer2 = nn.Sequential(QuantConv(nf + 1 * gc, gc, 3, padding=1, bias=True), nn.LeakyReLU(negative_slope=0.2, inplace=True))
        self.layer3 = nn.Sequential(QuantConv(nf + 2 * gc, gc, 3, padding=1, bias=True), nn.LeakyReLU(negative_slope=0.2, inplace=True))
        self.layer4 = nn.Sequential(QuantConv(nf + 3 * gc, gc, 3, padding=1, bias=True), nn.LeakyReLU(negative_slope=0.2, inplace=True))
        self.layer5 = nn.Sequential(QuantConv(nf + 4 * gc, nf, 3, padding=1, bias=True))
